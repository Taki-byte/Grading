...
if (isset($_GET['id']) && $_GET['id'] !== '') {
    $search = $_GET['id'];
    $stmt = $conn->prepare(
        "SELECT student_id, student_name, task_name, score, week, term, date
         FROM grades
...
<div class="sidebar">
    ...
    <a href="/grading-system/login.php?logout=1" class="logout-link">Logout</a>
</div>
...
<div class="main-content">
    ...
    <div class="divider">
        <?php if ($page > 1) { ?>
            <a href="view.php?id=<?= htmlspecialchars($search) ?>&page=<?= $page - 1 ?>">Previous</a>
        <?php } ?>
        <?php if ($page < $total_pages) { ?>
            <a href="view.php?id=<?= htmlspecialchars($search) ?>&page=<?= $page + 1 ?>">Next</a>
        <?php } ?>
    </div>
</div>
</body>
